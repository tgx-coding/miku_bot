...
NAPCAT_API = os.getenv("NAPCAT_API", "http://napcat:3000").rstrip("/")
...
MOOD_VALUE = {
    "FAVOURITE":{"min": 500, "name": "不可分离", "hint": "嫉妒病娇,完全顺从"},
    "LOVE":     {"min": 390, "name": "至死不渝", "hint": "有点病娇,乖巧"},
    "LIKE":     {"min": 200,  "name": "非常喜欢", "hint": "热情主动"},
    "FRIEND":   {"min": 100,  "name": "亲密伙伴", "hint": "开朗，亲密"},
    "NORMAL":   {"min": 50,  "name": "普通朋友", "hint": "礼貌，贴切"},
    "STRANGER": {"min": 0,   "name": "点头之交", "hint": "客气，保持距离"},
    "ANNOY":    {"min": -50, "name": "有点讨厌", "hint": "冷淡，高冷"},
    "HATE":     {"min": -100, "name": "极度厌恶", "hint": "毒舌，拒绝回答，辱骂"},
    "ENEMY":    {"min": -300,"name": "不共戴天", "hint": "完全无视，极度高冷"}
}
...
# ##猫葱 Bot 指令手册 (开发者专用)
# 权限： 仅限 `config.DEVELOPING_NUMBER` 配置的 QQ 号。
# 注意： 所有指令需配合 @机器人 触发。
# ### 1. 系统控制
# 禁言bot：
#   让猫葱在当前群聊不再说话
...
#    示例：
#        `@猫猫 好感+100` （支持多人同时 @）
#        `好感=520 12345678` （直接指定 QQ 号）
#        `好感-50` （配合 @ 使用）
# 3. 群组配额管理
#    指令： 
#       `加额度 [数值]` / `额度+[数值]`
#    示例： `加额度 5000` (当前群今日已消耗 Token 将减少 5000)
#    注意： 必须在目标群聊内触发。
# ### 4. Token 审计与统计
# `余额`  查询 SiliconFlow 等 API 的实际金钱/Token 余额。 
# `ds余额`  专门查询 DeepSeek 官网 API 余额。 
# `平均token`  统计决策层和输出层的平均消耗，用于优化提示词。 
# `清空token`  重置本地统计数据（不影响 API 余额，仅重置统计表）。 


# ## 🛡️ 猫葱 Bot 管理员手册 (群管理专用)
# **权限：** 群主、管理员或开发者。
# **注意：** 指令需 **@机器人** 触发，主要用于管理机器人行为。

# ### 1. 状态管理
# | 指令 | 说明 |
# | :--- | :--- |
# | `禁言bot` | 让机器人在当前群进入“面壁”状态，不再回复任何消息。 |
# | `解禁bot` | 解除禁言状态，恢复正常聊天。 |

# ### 2. 配额查询
# | 指令 | 说明 |
# | :--- | :--- |
# | `查询gp余额` | 或 `查询群token`。查看本群今日已用额度、累计消耗以及预计剩余对话次数。 |

# ### 3. 社交逻辑
# | 指令 | 说明 |
# | :--- | :--- |
# | `[marry:QQ号]` | 与特定成员结婚（AI 可触发，管理员可手动辅助逻辑调试）。 |
# | `[divorce:QQ号]` | 与特定成员离婚。 |
